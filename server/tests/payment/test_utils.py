...
@pytest.mark.parametrize("payment_hash", ["SomePayload", "And_AOther-WeridPaylo1d"])
def test_generates_ton_ref_hash(payment_hash: str) -> None:
    payload_cell = (
        begin_cell()
        .store_uint(0, 32)
        .store_snake_string(TonPaymentPayload.COMMENT_TEMPLATE.format(payment_hash))
        .end_cell()
    )
    payload_boc = payload_cell.to_boc()
    payload = base64.b64encode(payload_boc).decode("utf-8")

    ton_payment_payload = TonPaymentPayload(hash=payment_hash)

    assert ton_payment_payload.get_base64() == payload


@pytest.mark.parametrize("payment_hash", ["SomePayload", "And_Aother-WeirdPaylo1ad12"])
def test_parses_right_payload(payment_hash: str) -> None:
    tonapi_transaction = create_tonapi_transaction_mock(
        text=TonPaymentPayload.COMMENT_TEMPLATE.format(payment_hash)
    )

    ton_payment_payload = TonPaymentPayload.from_tonapi_transaction(tonapi_transaction)

    assert ton_payment_payload.hash == payment_hash
...
def test_from_tonapi_raises_if_wrong_op_name(op_name: str) -> None:
    tonapi_transaction = create_tonapi_transaction_mock(
        text=TonPaymentPayload.COMMENT_TEMPLATE.format("someHashie"), op_name=op_name
    )

    with pytest.raises(ValueError, match="op name is not text_comment"):
        TonPaymentPayload.from_tonapi_transaction(tonapi_transaction)
...
def test_from_tonapi_raises_if_wrong_comment_text() -> None:
    ...
    match = re.match(pattern=TonPaymentPayload.COMMENT_PATTERN, string=text)
